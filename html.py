...
class HTML(HTMLParser.HTMLParser):
    ...
    ## The top-most/root Tag object for a document/URL.
    root_ = None
    ...
    rate_limit_count_ = 0  # number of HTTP requests during this rate period

    ## Constructor
    #
    #  Create an instance of the HTML class, prepare the HTMLParser parent
    #  class for processing, and reset the root Tag, tag counter, and rate
    #  limiting.
    def __init__(self):
        self.parsed_data_ = []
    ...
    def parse_file(self, file_name):
        ...
        if file_name and len(file_name) > 0:
            ...
            try:
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: IOError while opening the \
                                             file %s - %s\n" %
                                             (str(file_name), str(e)))
                    self.debug_output_.flush()
    ...
    #  @param wait_for_rate_limiting True to block for rate limiting
    #         (default True).
    ...
    def parse_url(self, url, wait_for_rate_limiting=True):
        ...
        if url and len(url) > 0:
            ...
            try:
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: URLError while opening \
                                             the url %s - %s\n" %
                                             (str(url), str(e)))
                    self.debug_output_.flush()
            ...
            if (current_time - self.rate_limit_counter_time_) > \
    ...
    #
    #  @param url URL to parse.
    #  @param form_data a dictionary containing name/value pairs of the web
    #         form data.
    #  @param wait_for_rate_limiting True if the function should block for
    ...
    def parse_url_with_post_form(self, url, form_data,
        ...
        if url and len(url) > 0 and form_data:
            if self.debug_:
                self.debug_output_.write("HTML: Parsing the url with \
                                         POST form: %s\n" % (str(url)))
                self.debug_output_.flush()
            ...
            try:
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: ConnectionError while \
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: HTTPError while opening \
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: Timeout while opening the \
                                             url with form %s - %s\n" %
                                             (str(url), str(e)))
                    self.debug_output_.flush()
                raise Error("Error opening the url")
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: TooManyRedirects while \
                                             opening the url with \
                                             form %s - %s\n" %
                                             (str(url), str(e)))
                    self.debug_output_.flush()
            ...
            if self.debug_:
                self.debug_output_.write("HTML: No URL or form data \
                                         specified\n")
                self.debug_output_.flush()
        return False
    ...
    #  @param wait_for_rate_limiting True if the function should block for
    #         rate limiting (default True).
    #  @return True if the rate limit was handled properly, False if the rate
    #          is exceeded and we didn't wait.
    def check_rate_limiting(self, wait_for_rate_limiting=True):
        # Check rate limiting: time between requests
        current_time = datetime.datetime.now()
        if self.last_request_time_ and \
           (current_time - self.last_request_time_) < \
           datetime.timedelta(seconds=self.minimum_time_between_requests_):
        ...
        if self.rate_limit_count_ >= self.requests_per_minute_:
            if wait_for_rate_limiting:
                rate_delay = 60 - \
                    (current_time - self.rate_limit_counter_time_).seconds
                if rate_delay > 0:
    ...
    def parse(self, markup_text):
        ...
        if markup_text:
            ...
            try:
                ...
                if self.debug_:
                    self.debug_output_.write("HTML: HTMLParseError while \
                                             parsing the HTML - %s\n" % str(e))
                    self.debug_output_.flush()
        ...
        if self.debug_:
            self.debug_output_.write("HTML: finished parsing. \
                                     Position - %s\n" % str(self.getpos()))
            self.debug_output_.write("HTML: %s tags \
    ...
    #
    # @param tag the Tag object to store in the flat list, including it's
    #        children Tags.
    ...
    def handle_endtag(self, tag):
        ...
        self.current_tag_.data_ = \
            ''.join(self.current_tag_.string_concat_list_)
        if tag == 'em':
            self.current_tag_.parent_.string_concat_list_.\
                append(self.current_tag_.data_)
    ...
    def handle_comment(self, data):
        ...
        if data:
            ...
            data = data.replace("\r", "")
            if len(data) > 0:
                new_tag = Tag(name='comment', parent=self.current_tag_,
                              data=data)
                if not self.current_tag_.children_:
                    self.current_tag_.children_ = []
                self.current_tag_.children_.append(new_tag)
    ...
    def handle_decl(self, decl):
        ...
        if decl:
            ...
            decl = decl.replace("\r", "")
            if len(decl) > 0:
                new_tag = Tag(name='declaration', parent=self.current_tag_,
                              data=decl)
                if not self.current_tag_.children_:
                    self.current_tag_.children_ = []
                self.current_tag_.children_.append(new_tag)
        ...
        if data:
            ...
            if len(data) > 0:
                new_tag = Tag(name='unknown_declaration',
                              parent=self.current_tag_, data=data)
                if not self.current_tag_.children_:
                    self.current_tag_.children_ = []
                self.current_tag_.children_.append(new_tag)
    ...
    #  @param tag_attributes the tag attributes to search for.
    #         e.g. [('name', 'somename'), ('type', 'text/css')] (default None).
    #  @param tag_data the tag content to search for.
    #         e.g. "Title" (default None).
    #  @return the index in the flat list of the first matching tag,
    #          -1 if no match found.
    ...
    #  @param tag_data the tag content to search for.
    #         e.g. "Title" (default None).
    #  @param index the starting point in the flat list of tags to start
    #         looking (default 0).
    ...
    #  @return A list containing: [the tag type, nested list with tag attribute
    #          tuples, and the tag content], or None on error.
